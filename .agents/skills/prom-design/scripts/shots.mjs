...
async function loadChromium () {
  ...
  const projectPaths = [
    join(process.cwd(), 'project', 'node_modules', 'playwright-core'),
    join(process.cwd(), 'node_modules', 'playwright-core'),
  ]
  for (const p of projectPaths) {
    if (existsSync(p)) {
      try {
        const c = pick(await import('file:///' + p.replace(/\\/g, '/') + '/index.mjs'))
        if (c) return c
      } catch {}
      try {
        const c = pick(await import('file:///' + p.replace(/\\/g, '/') + '/index.js'))
        if (c) return c
      } catch {}
    }
  }
